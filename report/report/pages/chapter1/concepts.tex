\section{Concepts \& Terminology}
	...
	However as the overview progresses less comparisons will be made.  
	\newline
	\newline
	...
	the standards or provides a translation pair relevant that specific version of windows. 
	...
	it was quickly realised that this was an inefficient manner of storing configurations.  The Windows Registry solved this issue 
	by centralising configurations settings into a hierarchical database containing settings for low-level operating system settings 
	as well as settings for applications running on the platform.  
	\index{Interoperability} 
	\index{INI files} 
	\index{Registry} 
	\index{Dynamic link libraries}
	The advent of this technology and a central place for configuration provided an easy interface for a server 
	(domain controller) to apply settings.  The application of settings through this manner to groups of machines is known
	as group policy. 
	\index{Group Policy} 
	\index{Domain controller} 
	\newline
	\newline
	Given this brief overview of group policy, lets take a look at the "Gnu’s not Unix" (GNU) systems. 
	Since the conception of Linux in 1992 and the accompanied "Gnu’s not Unix" (GNU) applications, files still
	remain the de facto way of configuring these systems and their application preferences.   
	The style of these configuration files is somewhat similar to that of INI files, in that there is key/value pairs.
	\index{Group Policy} 
	\index{INI files}
	\newline
	\newline
	...
	\newline
	\newline
	Even though the underlying well-worn technologies that provide the back end implementation are primarily the same.  

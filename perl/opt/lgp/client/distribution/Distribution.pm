...
sub new 
{
	...
	my $self =
	{
		_packageManager => undef,
		_serviceController => undef,
		_serviceModifier => undef,
		_log => undef,
		_distrosServices => \%services,
		_packageManager => undef
	};
...
sub getServiceModifier
{
    my( $self ) = @_; 
	
    return $self->{ _serviceModifier };
}


sub getDistributionSpecificService
{
	my( $self , $service ) = @_;
	
	return $self->{ _distrosServices }->{ $service };
}
